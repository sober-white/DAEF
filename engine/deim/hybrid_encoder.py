...
from engine.extre_module.custom_nn.conv_module.wtdown import WTFusion
...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_channels = [hidden_dim for _ in range(len(feat_strides))]
        self.out_strides = feat_strides

        # channel projection
        self.input_proj = nn.ModuleList()
        if len(self.feat_strides) == 3:
            for in_channel in in_channels[2:]:
            ...
            for in_channel in in_channels[3:]:
                proj = nn.Sequential(OrderedDict([
                        ('conv', nn.Conv2d(in_channel, hidden_dim, kernel_size=1, bias=False)),
                        ('norm', nn.BatchNorm2d(hidden_dim))
                    ]))
        ...
        #WTFusion
        self.WTFusion = WTFusion(self.in_channels[0], self.in_channels[1], self.in_channels[2], self.hidden_dim, self.hidden_dim)

        # top-down fpn
        self.lateral_convs = nn.ModuleList()
        self.fpn_blocks = nn.ModuleList()
        for _ in range(len(self.feat_strides) - 1, 0, -1):
            ...
            self.fpn_blocks.append(
                RepNCSPELAN4(hidden_dim * 3, hidden_dim, hidden_dim * 2, round(expansion * hidden_dim // 2), round(3 * depth_mult), act=act) \
                if version == 'dfine' else CSPLayer(hidden_dim * 3, hidden_dim, round(3 * depth_mult), act=act, expansion=expansion, bottletype=VGGBlock)
            )

        # bottom-up pan
        self.downsample_convs = nn.ModuleList()
        self.pan_blocks = nn.ModuleList()
        for _ in range(len(self.feat_strides) - 1):
    ...
    def forward(self, feats):
        ...
        if len(self.feat_strides) == 3:
            proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats[2:])]
        else:
            proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats[3:])]
        ...
        if len(self.feat_strides) == 3:
            feats[2:] = proj_feats
            WT_feat = [F.interpolate(self.WTFusion(feats), scale_factor=2., mode='nearest'), self.WTFusion(feats)]
        else:
            feats[3:] = proj_feats
            WT_feat = [self.WTFusion(feats)]

        # broadcasting and fusion
        inner_outs = [proj_feats[-1]]
        for idx in range(len(self.feat_strides) - 1, 0, -1):
            feat_heigh = inner_outs[0]
            feat_low = proj_feats[idx - 1]
            feat_heigh = self.lateral_convs[len(self.feat_strides) - 1 - idx](feat_heigh)
            inner_outs[0] = feat_heigh
            upsample_feat = F.interpolate(feat_heigh, scale_factor=2., mode='nearest')
            inner_out = self.fpn_blocks[len(self.feat_strides)-1-idx](torch.concat([upsample_feat, feat_low, WT_feat[idx-1]], dim=1))
            inner_outs.insert(0, inner_out)

        outs = [inner_outs[0]]
        for idx in range(len(self.feat_strides) - 1):
            feat_low = outs[-1]
